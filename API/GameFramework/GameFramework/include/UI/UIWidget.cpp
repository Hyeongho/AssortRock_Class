...
CUIWidget::CUIWidget() : m_ZOrder(0), m_Visibility(true), m_MouseHovered(false)
{
}
...
{
	if (m_Pos.x <= MousePos.x && MousePos.x <= m_Pos.x + m_Size.x && m_Pos.y <= MousePos.y && MousePos.y <= m_Pos.y + m_Size.y)
	{
		if (!m_MouseHovered)
		{
