...
private:
	CAVLTreeNode() : m_Left(nullptr), m_Right(nullptr), m_Parent(nullptr), m_Next(nullptr), m_Prev(nullptr)
	{
...
private:
	CAVLTreeNode<typename KEY, typename VALUE>* m_Node;
	...
	*/
	...
	iterator erase(const iterator& iter)
	{
		...
		if (!iter.m_Node->m_Left && !iter.m_Node->m_Right)
		{
			...
			// 그런데 루투노드가 리프노드라는 말은 이 노드 1개만 남았다는 것이다.
			if (!Parent)
			{
				delete iter.m_Node;
			...
			// 지우려는 노드가 부모노드의 왼쪽 노드인지 오른쪽 노드인지를 판단하여 부모와의 연결을 끊어준다.
		...
		// 만약 왼쪽 노드가 있을 경우 왼쪽 노드에서 가장 큰 노드를 찾아서 지울 노드의 값을 변경해주고 찾아준 노드를 제거한다.
		if (iter.m_Node->m_Left)
		{
			...
			// 찾아준 노드를 제거해야하기 때문에 부모로부터 연결을 끊고 제거해주도록 한다.
			// 단, 찾아준 노드가 왼쪽 자식노드가 있을수고 있으므로
			...
			// 왼쪽 자식노드가 있을 경우라면 부모로 MaxNode의 부모를 지정해주도록 한다.
			if (LeftChild)
			{
		...
		// 지울 노드의 오른쪽 노드만 존재할 경우 오른쪽 노드에서 가장 작은 노드를 찾아온다.
		...
		// 오른쪽 자식노드가 있을 경우라면 부모로 MaxNode의 부모를 지정해주도록 한다.
		if (RightChild)
		{
...
private:
	PNODE insert(const KEY& key, const VALUE& value, PNODE Node)
	{
		...
		if (Node->first > key)
		{
			// 만약 기준노드의 왼쪽 자식노드가 있다면 그 왼쪽 자식노드를 기준노드로 하여 다시 탐색을 하게 한다.
			if (Node->m_Left)
			{
				return insert(key, value, Node->m_Left);
			}

			// 더이상 왼쪽 자식노드가 없을 경웅 이 위치에 새로 노드를 생성하여 추가해주어야 한다.
		...
		// 만약 오른쪽 자식노드가 있을 경우 기준노드를 오른쪽 자식노드로 하여 탐색하게 한다.
		if (Node->m_Right)
		{
			return insert(key, value, Node->m_Right);
		}

		// 더이상 오른쪽 자식노드가 없을 경우 이 위치에 새로 노드를 생성하여 추가해주어야 한다.
		PNODE NewNode = new NODE;
	...
	{
		// 기준노드가 nullptr일 경우 더이상 탐색할 노드가 없으므로 end를 리턴한다.
		if (!Node)
		{
			return end();
		}
	...
	PNODE RotationLeft(PNODE Node)
	{
		...
		// 기준노드의 오른쪽 자식노드의 왼쪽 자식 노드를 얻어온다.
		PNODE RightLeftChild = RightChild->m_Left;

		// 기준노드의 오른쪽 자식노드로 오른쪽 자식노드의 왼쪽자식노드로 이동시켜준다
		...
		// 그러므로 기준노드의 자리에 RightChild가 오게 되므로 루트노드를 RightChild로 교체해주어야 한다
		else
		{
			m_Root = RightChild;
		}
	...
	PNODE RotationRight(PNODE Node)
	{
		...
		// 기준노드의 왼쪽 자식노드로 왼쪽 자식노드의 오른쪽자식노드로 이동시켜준다
		Node->m_Right = LeftRightChild;
		...
		// 기준노드의 부모노드가 없다면 기준노드는 루트노드라는 의미이다.
		// 그러므로 기준노드의 자리에 LeftChild가 오게 되므로 루트노드를 LeftChild로 교체해주어야 한다
		else
		{
			m_Root = LeftChild;
		}
