...
private:
	void BFSAlgorithm(int index)
	{
		CQueue<CGraphNode<T>*> queue;

		CGraphNode<T>* CurrentNode = m_NodeArray[index];
		CGraphNode<T>* NextNode = nullptr;

		CurrentNode->m_Visit = true;

		queue.push(CurrentNode);

		while (!queue.empty())
		{
			CGraphNode<T>* front = queue.front();

			std::cout << front->m_Data << " - ";

			queue.pop();

			for (int i = 0; i < front->m_Size; i++)
			{
				NextNode = front->m_EdgeArray[i]->m_Node;

				if (!NextNode->m_Visit && (NextNode != nullptr))
				{
					queue.push(NextNode);

					NextNode->m_Visit = true;
				}
			}
		}
	}

	void DFSAlgorithm(int index)
	{
		CStack<CGraphNode<T>*> stack;
		...
		while (!stack.empty())
		{
			...
			{
				NextNode = top->m_EdgeArray[i]->m_Node;
